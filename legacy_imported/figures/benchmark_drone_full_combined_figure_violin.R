...
#   source path   Manuscript figures/benchmark_drone_full_combined_figure_violin.R
#   ref           main @ 671e56f
#   sha256        8d2dab0fdf7987f41698c4f82a78d16db7160ee74590d21d0b851648d89036f6
#
#   why           Figure 4 alternative rendering as violins.
#
...
Learner_bp_grey <- ggplot(data = BENCH, mapping = aes (x=learner, y=Accuracy, group = learner))+ #geom_jitter(size=0.25)+ 
  xlab("Learner") + 
  ylim(0.65,1)+ scale_x_discrete(label = learner_labels) + labs(x = "Learner", y = "Overall Accuracy") + geom_jitter(size=0.25)+geom_violin(fill=c("grey38"))+theme_fancy()
p1 <-plot(Learner_bp_grey)

Learner_bp_grey <- ggplot(data = BENCH, mapping = aes (x=learner, y=Accuracy, group = learner))+ 
  xlab("Learner") + stat_boxplot(fill=c("grey38"),outlier.shape = NA)+theme_fancy()+
  ylim(0.65,1)+ scale_x_discrete(label = learner_labels) + labs(x = "Learner", y = "Overall Accuracy") +geom_jitter(size=0.25) 
p1a <-plot(Learner_bp_grey)
...
input_data_bp_grey <- ggplot(data = BENCH, mapping = aes (x=input_data, y=Accuracy, group = input_data))+ #geom_jitter(size=0.25)+ 
  xlab("Input data set") + stat_boxplot(fill=c("grey80"),outlier.shape = NA)+theme_fancy()+
  ylim(0.65,1)+ scale_x_discrete(label = ilabels) + labs(x = "Input data set", y = "Overall Accuracy") 
p2 <- plot(input_data_bp_grey)
...
input_data_bp_prosopis2 <- ggplot(data = CONFUSION, mapping = aes (x=input_data, y=Accuracy, group = input_data))+ #geom_jitter(size=0.25)+ 
  xlab("Input data") + stat_boxplot(fill=c("grey80"),outlier.shape = NA)+theme_fancy()+
  ylim(0.65,1)+ scale_x_discrete(label = ilabels) + labs(x = "Input data set", y = "Prosopis Overall Accuracy") 
...
ggplot2::ggsave(
  P_All3,
  # filename = "/plots/test.png",
  filename = paste0("output_data/Manuscript figures/Combined_drone_accuracy_figure_v6.jpg"),
  width = 16,
  height = 8,
  units = "cm"
) 
