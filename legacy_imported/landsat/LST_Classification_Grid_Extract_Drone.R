...
#   source path   Landsat/LST_Classification_Grid_Extract_Drone.R
#   ref           main @ 671e56f
#   sha256        34a27080e7d00e0f867a992e54e24c9c1673513a3e12727f02a34d6d2b218cca
#
...
WV2_RF_LST <- rast("E:/Glenn/Botswana/Satellite_Data/WV2/1_6_m_mosaic/RF_WV2_additional.tif")
...
LST_Predict <- rast("E:/Glenn/Botswana/MLR3_classifications/Struizendam_2_Predict_Map (2).tif")
...
#NDVITrend<- rast("E:/Glenn/Botswana/Satellite_Data/Trend_Image_Data/LST/Trend_Img_NDVI_2000_2022b.tif")
#LST <-terra::project(NDVITrend, y="EPSG:32734")


## NB You cant use exact extract with SpatVector so re-importing it a a spatial DF
LSTWV_Grid_SF <- read_sf(dsn = 'E:/Glenn/Botswana/Satellite_Data/LST/LST_Grids', layer = "WV2_LST_grid")
S1_Grid_SF <- read_sf(dsn = 'E:/Glenn/Botswana/Satellite_Data/LST/LST_Grids', layer = "Struizendam_1_LST_grid")
LST_Grid_SF <- read_sf(dsn = 'E:/Glenn/Botswana/Satellite_Data/LST/LST_Grids', layer = "Struizendam_2_LST_grid")
S3_Grid_SF <- read_sf(dsn = 'E:/Glenn/Botswana/Satellite_Data/LST/LST_Grids', layer = "Struizendam_3_LST_grid")
S4_Grid_SF <- read_sf(dsn = 'E:/Glenn/Botswana/Satellite_Data/LST/LST_Grids', layer = "Struizendam_4_LST_grid")
B1_Grid_SF <- read_sf(dsn = 'E:/Glenn/Botswana/Satellite_Data/LST/LST_Grids', layer = "Bokspits_1_LST_grid")
B2_Grid_SF <- read_sf(dsn = 'E:/Glenn/Botswana/Satellite_Data/LST/LST_Grids', layer = "Bokspits_2_LST_grid")
B3_Grid_SF <- read_sf(dsn = 'E:/Glenn/Botswana/Satellite_Data/LST/LST_Grids', layer = "Bokspits_3_LST_grid")


#---2. Extract data for WV2 image from LST grid


WV2_RF_LST_Extract <- exact_extract(WV2_RF_LST,LSTWV_Grid_SF,"mode")
names(WV2_RF_LST_Extract) <- c('mode')
WV2_RF_LST_Extract_DF <-bind_cols(LSTWV_Grid_SF,WV2_RF_LST_Extract)

WV2_RF_LST_Extract <- exact_extract(WV2_RF_LST,LSTWV_Grid_SF,"majority" )
names(WV2_RF_LST_Extract) <- c('majority')
WV2_RF_LST_Extract_DF <- dplyr::mutate(WV2_RF_LST_Extract_DF, majority = WV2_RF_LST_Extract)

WV2_RF_LST_Extract <- exact_extract(WV2_RF_LST,LSTWV_Grid_SF,"frac" )# calculates fraction cover for each classification value
#a column is added with the FVC for each vegetation type frac1, frac2 etc...
WV2_RF_LST_Extract_DF <- dplyr::mutate(WV2_RF_LST_Extract_DF, WV2_RF_LST_Extract)

WV2_full_Extract_DF <- WV2_RF_LST_Extract_DF

WV2_full_Extract_DF[is.na(WV2_full_Extract_DF)] <- 0

write_xlsx(WV2_full_Extract_DF, "E:/Glenn/Botswana/R_Scripts/Glenn-Prosopis-ML/data_out/WV2_full_Extract_DF_LST_Grid.xlsx")
...
Drone_extract_prosopis_cover_LSTGrid <- B1_full_Extract_DF%>% dplyr::select (FID,majority,frac_1)
...
Drone_extract_prosopis_cover_LSTGrid <- rbind(Temp,Drone_extract_prosopis_cover_LSTGrid)
...
write_xlsx(Drone_extract_prosopis_cover_LSTGrid, "E:/Glenn/Botswana/R_Scripts/Glenn-Prosopis-ML/data_out/Drone_extract_prosopis_cover_LSTGrid.xlsx")
