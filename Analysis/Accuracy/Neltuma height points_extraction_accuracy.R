...
# Now for separate Neltuma points 
...
Bokspits_1_points<- read_sf(dsn = 'input_data/', layer = "Bokspits_1_Neltuma_height_points_20b")
Bokspits_1_points
...
Neltuma_df2 <- Bokspits_1_Drone_Extract_DF  %>% dplyr::select (fid,majority,frac_1, frac_2, frac_3,frac_4,frac_5,frac_6)
...
## NB You cant use exact extract with SpatVector so importing it a a spatial DF
Struizendam_1_points<- read_sf(dsn = 'input_data/', layer = "Struizendam_1_Neltuma_height_points_20b")
...
Neltuma_height_df <- rbind(Neltuma_df2,df5)
#Neltuma_height_df[, 'majority'] <- as.factor(Neltuma_height_df[, 'majority'])
...
P5 <- ggplot(Neltuma_height_df, aes(x=factor(majority))) +  geom_histogram(stat= "count")+ scale_color_viridis(discrete=TRUE) +
