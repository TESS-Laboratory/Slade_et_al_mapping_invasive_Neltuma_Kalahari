...
p2 <- ggplot(aes(x=factor(WV2)), data = Prospis_predict3)+ geom_bar(stat = "count")+ 
  scale_x_discrete(labels=c("1" = "Prosopis", "2"="BG", "3" = "Grass", "4" = "F","5" = "VE","6" = "RT/OW","7" = "OW"))+
...
ggsave(
  ...
  filename = "output_data/All data sets accuracy comparison with drone_with count.png",
  width = 16,
  height = 24,
  units = "cm"
)
...
Prosopis_predict <- filter(Full_S2,S2 == "1")
Prosopis_predict <- filter(Prosopis_predict, majority=="2")

p9 <- ggplot(aes(x = frac_1 ), data = Prosopis_predict) + 
