...
x <- build_cube_LS8_T(site_name = "LS8")
x
#plot(x)
#x <- rast("data_out/LS8/LS8_stack.tif") #access the intermediate layer

v <- build_ml_df_LS8(cube = x,
                     site_name = "LS8",
                     df_type="grid")

#v <- "data_out/LS8/LS8ML_in_grid_level.rds"
v
#================ ML pipeline ================
# Auto ML ------------------

task <- build_task(v, site_name="LS8")
...
writeRaster(mod.pred.fac, filename = paste0("data_out/",site_name,"/",site_name,"_Predict_Map_WV2_Timeseries_10P.tif"), overwrite=TRUE)


### Exporting Data ------
# Exporting confusion matrix
as.data.frame(res.preds)
write.xlsx(res.preds, paste0("data_out/Confusion/res.preds_svm_",site_name,"_WV2_Timeseries_10P.xlsx"), rowNames=FALSE)
...
write.xlsx(b, paste0("data_out/Bench/MLR_bench_results_",site_name,"_WV2_Timeseries_10P.xlsx"), rowNames=FALSE)


#Exporting benchmark plot
ggsave(
  p2,
  filename = paste0("data_out/Figures/benchmark_",site_name,"_model_classif_ac_WV2_Timeseries_10P.png"),
  width =16,
  height = 8,
  units = "cm"
)
...
ggsave(
  p3,
  filename = paste0("data_out/Figures/Final_",site_name,"_model_classif_ac_WV2_Timeseries_10P.png"),
  width =16,
  height = 8,
  units = "cm"
)
