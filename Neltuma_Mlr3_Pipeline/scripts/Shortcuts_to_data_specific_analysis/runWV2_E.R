...
#x <- build_cube_WV2(site_name = "WV2")

x <- rast("data_out/WV2/WV2_stack.tif") #access the intermediate layer

v <- build_ml_df_WV2(cube = x,
                 site_name = "WV2",
                 df_type="grid")

#v <- "data_out/WV2/WV2ML_in_grid_level.rds"
v
#================ ML pipeline ================
# Auto ML ------------------

task <- build_task(v, site_name="WV2")
